...
| `npm run qa:analysis-engine` | Standalone package ESM build, public API, Snapshot validation, semantic consistency, migration audit, and three JSON Schemas. |
...
| Standalone analysis engine | Fact/Feature/Hypothesis stay separate; current Snapshot is validated independently; schema versions and migrations are explicit; process fidelity is honest; expert contracts remain machine-valid. | `npm run build`, `npm run qa:analysis-spec`, `npm run qa:analysis-engine`, `npm run qa:snapshot-contract`. |
