...
| Standalone analysis engine | Fact/Feature/Hypothesis stay separate; current Snapshot is validated independently; scene and features are deterministic and traceable; schema versions and migrations are explicit; process fidelity is honest. | `npm run build`, `npm run qa:analysis-spec`, `npm run qa:analysis-engine`, `npm run qa:analysis-features`, `npm run qa:snapshot-contract`. |
