...
function stringifyCanonical(value: unknown): string {
  ...
  if (typeof value === "object") {
    ...
    return `{${Object.keys(record).sort(compareStrings).map((key) => `${JSON.stringify(key)}:${stringifyCanonical(record[key])}`).join(",")}}`;
  }
...
function utf8Bytes(input: string): Uint8Array {
  ...
  for (const character of input) {
    const rawCodePoint = character.codePointAt(0) ?? 0;
